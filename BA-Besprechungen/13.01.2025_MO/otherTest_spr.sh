...
do
	...
	sed -i "$((56 + 6 * $i))a phi_$i[i][k] = 0.25 * ( phi_$i[i][k-1] + phi_$i[i+1][k] + phi_$i[i][k+1] + phi_$i[i-1][k]);" $SOURCE
	sed -i "$((60 + 7 * $i))a phi_$i[i+1][0] = phi_$i[i][NK-1];" $SOURCE
	sed -i "$((75 + 8 * $i))a delete[] phi_$i;" $SOURCE
