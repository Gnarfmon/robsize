...
int main(int argc, char** argv) {
  ...
  bool debug = false;
  if(argc < 3) {
    printf("Usage: %s <NI> <CPU FREQUENCY in KHz> <debug>\n",argv[0]);
    exit(1);
  }
  if(argc >= 4 && atoi(argv[3])) debug = true;
  NI = atoi(argv[1]);
...
double cycles_counted = (double)(result - diff) / NITER / NI;
  ...
  printf("Cycles per high-level iteration calculated: %lf | counted: %lf \n", cycles_calculated, cycles_counted);
