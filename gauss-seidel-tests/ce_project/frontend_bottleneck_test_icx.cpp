...
int main(int argc, char** argv) {
 ...
 // NK = atoi(argv[2]);
  ...
  for (int i=0; i<NI; ++i) {
      phi[i][0] = (double)i * 0.2;
      phi[i][1] = (double)i + 0.2;
  }
  ...
  do {
    ...
    for(int it=0; it<NITER; ++it) {
      // benchmark loop
      for (int i=1; i<NI; ++i) {
	      rho[i][0] = phi[i][0] + phi[i][1];
	      rho[i][1] = phi[i][0] * phi[i][1];
      }
  ...
  double cycle_calc = (double)(wct_end-wct_start)*2400000000/NITER/NI/NK;
